...
\begin{frame}[t]{A recap on the SO splitting}
    ...
    \begin{columns}[c]
        ...
        \end{column}%
        \begin{column}{0.5\linewidth}
            Deviations from the trend are found due to:
            \begin{enumerate}
...
\begin{frame}[c]{Physics case}
    \textbf{Transfer} reactions to probe single-particle occupancies in \iso{20}{O}.
    \begin{columns}[c]
        \begin{column}{0.5\linewidth}
            \mycolorbox[1]{box2}{
                \boxitem{1} $Z = 6$ SO gap with $\iso{20}{O}(\iso{}{d},\iso{3}{He})\iso{19}{N}$\newline
                \includegraphics[width=0.75\linewidth]{figures/z6_gap_juan_tese.png}\newline
                {\itshape\footnotesize J. Lois-Fuentes, PhD thesis (2023)}
            }
        \end{column}%
        \begin{column}{0.5\linewidth}
            \begin{figure}
                \centering
                \includegraphics[width=0.8\linewidth]{figures/sm_20O.pdf}
                \caption{IPSM of \iso{20}{O}}
            \end{figure}
        \end{column}
    \end{columns}
    \vspace*{-0.65cm}
    \mycolorbox[1]{box3}{
        \boxitem{2} $N = 6$ SO gap through $\iso{20}{O}(\iso{}{d},\iso{}{t})\iso{19}{O}$ main objective of this analysis
    }
...
\begin{frame}[c]{Experimental setup}
    ...
    \begin{figure}
        ...
        % \centering
        \begin{tikzpicture}
            \node (main) at (0,0) {\includegraphics[width=0.5\linewidth]{figures/mini_setup.pdf}};
...
\section{Results}
\begin{frame}{Results: E$_{\text{x}}$ spectrum}
    \begin{figure}
        \centering
        \includegraphics[width=0.85\linewidth]{figures/ex.pdf}
    \end{figure}
    \begin{columns}[c]
        \begin{column}{0.5\linewidth}
            \mycolorbox[1]{box3}{
                11 observed states!\\
                (p,d) background at high E$_{\text{x}}$
            }
        \end{column}%
        \begin{column}{0.5\linewidth}
            \mycolorbox[1]{box1}{
                $T$ assigments based on comparison with $\iso{20}{O}(\iso{}{d}, \iso{3}{He})\iso{19}{N}$
            }
        \end{column}
    \end{columns}
...
\begin{frame}[c]{Results: cross-sections}
    \begin{columns}[c]
        \begin{column}{0.5\linewidth}
            \begin{tikzpicture}
                \node (main) at (0,0){
                    \includegraphics[width=0.75\linewidth]{figures/ang_slide0.pdf}
                };
                \node(top) at ([xshift=1cm,yshift=2cm]main.east) {
                    \includegraphics[width=0.3\linewidth]{figures/19O_0d52.pdf}
                };
                \node(bottom) at ([xshift=1cm,yshift=-1.5cm]main.east) {
                    \includegraphics[width=0.3\linewidth]{figures/19O_1s12.pdf}
                };
            \end{tikzpicture}
        \end{column}%
        \begin{column}{0.5\linewidth}
            \mycolorbox{box4}{
                \textbf{Ground-state} perfectly reproduced by $\ell = 2$ line
            }\\[2cm]
            \mycolorbox{box2}{
                Lowly populated $\ell = 0$
            }
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}[c]{Results: cross-sections}
    \begin{columns}[c]
        \begin{column}{0.5\linewidth}
            \includegraphics[width=0.85\linewidth]{figures/ang_slide1.pdf}
        \end{column}%
        \begin{column}{0.5\linewidth}
            \mycolorbox{box4}{
                \textbf{Ground-state} perfectly reproduced by $\ell = 2$ line
