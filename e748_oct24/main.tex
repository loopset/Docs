...
\section{Results}
\begin{frame}[c]{Resuls: Elastic \texorpdfstring{\iso{10,12}{Be}(d,d)\iso{10,12}{Be} }{10,12Be(d,d)10,12Be}}
    \only<+>{
        \begin{columns}[c]
            \begin{column}{0.5\linewidth}
                \begin{tikzpicture}
                    \node[anchor=south west,inner sep=0] (image) at (0,0) { \includegraphics[width=1\linewidth]{figures/10Be_dd_fit.eps}
                    };
                    \myscope[false]{
                        \node at (0.25, 0.6) {\iso{10}{Be}};
                    }
                \end{tikzpicture}
                \begin{tikzpicture}
                    \node[anchor=south west,inner sep=0] (image) at (0,0) { \includegraphics[width=1\linewidth]{figures/12Be_dd_fit.eps}
                    };
                    \myscope[false]{
                        \node at (0.25, 0.6) {\iso{12}{Be}};
                    }
                \end{tikzpicture}
            \end{column}%
            \begin{column}{0.5\linewidth}
    ...
    \only<+>{
        ...
        \begin{equation*}
            \frac{d\sigma}{d\Omega} = \frac{N}{N_{\text{beam}} \textcolor{red}{N_{\text{targets}}} \textcolor{red}{\epsilon} \Delta \Omega} = \frac{N}{N_{\text{beam}} \textcolor{red}{\alpha} \epsilon_{\text{np}} \Delta \Omega}
        \end{equation*}
        \begin{columns}[c]
            \begin{column}{0.5\linewidth}
                \mycolorbox[1]{box2}{
        ...
        \end{columns}
        \mycolorbox[1]{box2}{
            From \textbf{npsimu}: $\epsilon_{\text{np}} = \epsilon_{\text{geo, MUST2}} \cdot \epsilon_{\text{reconstruction}} \cdot \textcolor{magenta}{\epsilon_{\text{geo, ZDD}}}$\\
            Agglutinate missing factors: $\alpha = N_{\text{targets}} \cdot \epsilon_{\text{instrinsic, ZDD}} \cdot \epsilon_{\text{other?}}$
        }
    }
\end{frame}

\begin{frame}[t]{About DWBA calculations}
    % The DWBA approximations simplifies the single-nucleon transfer xs by considering it a \textit{small} perturbation of the \textbf{elastic} process.
    % OMPs with imaginary parts are introduced to represent the missing flux from the elastic scattering.
    \begin{columns}[c]
        \begin{column}{0.5\linewidth}
            \mycolorbox[0.95]{box2}{
                \raisebox{0.15em}{\enumitem{1}} \textbf{OMPs} for elastic
                \begin{itemize}
                    \item \iso{10,12}{Be} + d: Haixia
                    \item \iso{9,11}{Be,Li} + t,\iso{3}{He}: Pang and HT1p
                \end{itemize}
            }
        \end{column}%
        \begin{column}{0.5\linewidth}
            \mycolorbox[0.95]{box3}{
                \raisebox{0.15em}{\enumitem{2}} \textbf{Light} overlap.
                Usually determines the xs strength.
                \begin{itemize}
                    \item $\langle \text{t} \vert \text{d} + \text{n}\rangle$
                    \item $\langle \iso{3}{He} \vert \text{d} + \text{p}\rangle$
                \end{itemize}
                From GFMC calculations (\textit{ab initio})
            }
        \end{column}
    \end{columns}
    \mycolorbox[0.95]{box4}{
        \small{
            \raisebox{0.15em}{\enumitem{3}} Hea\textbf{}vy overlap $\langle \iso{10,12}{Be}\vert\iso{9,11}{Be,Li} \otimes \text{n},\text{p}\rangle$. Two WS prescriptions:
            \begin{itemize}
                \item Standard $r_0 = \qty{1.25}{\femto\m}$
                \item \textit{N. Timofeyuk PRC 81 (2010)} Source Term Approach (STA): includes in a phenomenological way some correlations among nucleons. $r_0$ for each channel.
            \end{itemize}}
    }
\end{frame}
        ...
        \begin{column}{0.5\linewidth}
            \begin{tikzpicture}[
                ...
                \begin{axis}[
                    ...
                    \addplot+ [mark=none, densely dashdotted, thick] coordinates {(\pgfkeysvalueof{/pgfplots/xmin}, 2.511) (\pgfkeysvalueof{/pgfplots/xmax}, 2.511)};
                    \addplot+ [mark=none, densely dashdotted, thick] coordinates {(\pgfkeysvalueof{/pgfplots/xmin}, 1.503) (\pgfkeysvalueof{/pgfplots/xmax}, 1.503)};
...
\begin{frame}[t]{Results: \texorpdfstring{\iso{12}{Be}(d,\iso{3}{He})\iso{11}{Li} }{12Be(d,3He)11Li}}
            ...
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    \only<3>{
                        ...
                        %% Fillings
                        % 1 Left
                        \addplot[pink1] fill between [of=laxis and left];
                        % 2 Middle
                        \addplot[green1] fill between [of=left and right];
                        % 3 Right
                        \addplot[brown1] fill between [of=right and raxis];
                    }
                    % \legend{stdWS, STA}
                \end{axis}
            \end{tikzpicture}
        ...
        \begin{column}{0.5\linewidth}
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    %% Line 1
                    \addplot[teal, ultra thick, name path=unity] coordinates {(\pgfkeysvalueof{/pgfplots/xmin}, 1) (\pgfkeysvalueof{/pgfplots/xmax}, 1)};
                    ...
                    \only<4>{
                        ...
                        \path[name path=raxis] (\pgfkeysvalueof{/pgfplots/xmax}, 0) -- ( \pgfkeysvalueof{/pgfplots/xmax}, \pgfkeysvalueof{/pgfplots/ymax});
                        %% Middel separation
                        \addplot[black, thick, name path=middle] coordinates {(1.5, \pgfkeysvalueof{/pgfplots/ymin} ) (1.5, \pgfkeysvalueof{/pgfplots/ymax})};
                        %% Fillings
                        % 1 Left
                        \addplot[brown1] fill between [of=laxis and middle];
                        % 2 Right
                        \addplot[blue1] fill between [of=middle and raxis];
                    }
                \end{axis}
            \end{tikzpicture}
        \end{column}
    ...
    \only<4>{
        \begin{columns}
            \begin{column}{0.5\linewidth}
                \mycolorbox{box2}{
                    Great prediction for \iso{9}{Li} but understimates SF for \iso{9}{Be}!
                }
            \end{column}%
            \begin{column}{0.5\linewidth}
                \mycolorbox{box3}{
                    \textbf{Missing} predictions for \iso{9}{Li_{1st}} and \iso{11}{Li}
                }
            \end{column}
        \end{columns}
    }
...
\begin{frame}[c]{Conclusions}
    ...
    \mycolorbox{box2}{
        Need more theoretical insights to check validity of STA
    }%
\end{frame}
