...
\begin{frame}{A glance at the analysis}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\linewidth}
			\mycolorbox[1]{boxGreen}{
				$E_{x}$ resolution in very good agreement with simulations!
			}
		\end{column}
	\end{columns}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.45\linewidth}
			\mycolorbox[0.9]{box4}{
				...
				$\Rightarrow$ compare $\beta_2$ with E. Khan
			}
		\end{column}
	\end{columns}
...
\begin{frame}{About ESPEs}
	Effective single-particle energies (ESPEs) are needed to locate the $1p_{1/2}$ and $1d_{5/2}$ orbits: \textbf{Baranger's formula}
	\begin{equation*}
...
\begin{frame}{Results: \iso{20}{O}(d,t)}
	Assuming all $\Delta L = 1$ states below $E_{x} = \qty{10}{\MeV}$ are $p_{1/2}$...
	\begin{figure}
	...
	\begin{columns}[c]
		...
		\begin{column}{0.5\linewidth}
			\mycolorbox{box2}{
				Higher $1p_{1/2}$ occupation according to exp.
			}
		\end{column}
	\end{columns}
...
\begin{frame}{Future work}
	...
	\mycolorbox{box4}{
    Ask theoriticians if interactions can be tuned for our data
	}
	\vspace{1em}

	\mycolorbox{box1}{
    Continue to investigate (p,d)
	}%
\end{frame}
\end{document}
